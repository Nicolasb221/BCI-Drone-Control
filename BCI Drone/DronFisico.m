...
testCommand_iz = [1,-1,0,0,0, 0.5];
testCommand_de = [1,0.5,0,0,0, 0.5];
testCommand_s = [1,0,0,0,0, 0.5];
    ...
    if z == 0       
        ...
        elseif vec_mov(samples_acquired/ventana) == vec_mov(contador)
            ...
            if vec_mov(contador)==1
                if tags_modelo_DronFisico(contador_t) == 1
                    Estimulo = 1;
                    SequenceNumber = tic;
                    SequenceNumber = TakeOff(SequenceNumber, controlChannel, stateChannel);
                    if SequenceNumber ~= -1
                        pause(1);
                        [DroneState, OptionData, SequenceNumber] = ...
                            ControlPannel (SequenceNumber, controlChannel, stateChannel, testCommand_s, 0);
                    end
                else
                    vec_mov=[vec_mov(1:contador-1) vec_mov(contador) vec_mov(contador:end)];
                end
                if samples_acquiredl~=768
                    contador_t = contador_t+1;
                end

            end
            %Movimiento 2 Dderecha
            if vec_mov(contador)==2
                if tags_modelo_DronFisico(contador_t) == 2
                    Estimulo = 4;
                    if SequenceNumber ~= -1
                        pause(1);
                        [DroneState, OptionData, SequenceNumber] = ...
                            ControlPannel (SequenceNumber, controlChannel, stateChannel, testCommand_de, 0);
                    end
                else
                    vec_mov=[vec_mov(1:contador-1) vec_mov(contador) vec_mov(contador:end)];
                end
            ...
            if vec_mov(contador)==3
                if tags_modelo_DronFisico(contador_t) == 4
                    Estimulo = 5;
                    if SequenceNumber ~= -1
                        pause(1);
                        [DroneState, OptionData, SequenceNumber] = ...
                            ControlPannel (SequenceNumber, controlChannel, stateChannel, testCommand_iz, 0);
                    end
                else
                    vec_mov=[vec_mov(1:contador-1) vec_mov(contador) vec_mov(contador:end)];   
                end
            ...
            if vec_mov(contador)==4
                if tags_modelo_DronFisico(contador_t) == 3
                    Estimulo = 6;
        ...
        tags_modelo_DronFisico(contador_t) = Modelo_Voluntario_5.predictFcn(vec_feat);
...
tags_modelo_DronFisico(end-10:end) = [];
%guardar variable
tags_modelo_DronFisico;

%predicciones... Matriz de confusión

Correcto_Dron=[1,2,4,3];
ctn_Dron=1;
vec_corr_Dron =[];
for f=1:length(tags_modelo_DronFisico)
    if Correcto_Dron(ctn_Dron)==tags_modelo_DronFisico(f)
        vec_corr_Dron(f)=Correcto_Dron(ctn_Dron);
        ctn_Dron=ctn_Dron+1;
    else
        vec_corr_Dron(f)=Correcto_Dron(ctn_Dron);
    end 
...
Confus_Dron = confusionmat(vec_corr_Dron,tags_modelo_DronFisico)
